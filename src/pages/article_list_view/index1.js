...
class ArticleListView extends React.Component {
  ...
  componentWillMount(){
    axios({
      ...
      let nowDatas = this.state.datas
      this.setState({
        datas:nowDatas.concat(res.data.data)
      })
    })
  }
  componentDidMount() {
...
>{obj.title}</div>
            <div className="article-content-wrap" dangerouslySetInnerHTML={{__html:obj.content}}></div>
                <div style={{ lineHeight: "20px",color:'grey', height:"20px",display:"-webkit-box" }}>
                  <div style={{height:20,width:20, backgroundSize: "contain",backgroundImage:`url(${obj.headimgurl})`}} />
                  <div style={{marginLeft:10,fontSize:11}}>{obj.nickname}</div>
                  <div style={{display:"inline-flex",fontSize:"11px", marginLeft:"10px"}}>
                      <div style={{marginLeft:"20px"}}>{this.formatTime(obj.edit_time)}</div>
                      ...
                      <div style={{marginLeft:"20px"}}>{obj.comment}</div>
                  </div>
                </div>
            </div>
        )
